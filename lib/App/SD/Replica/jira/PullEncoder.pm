package App::SD::Replica::jira::PullEncoder;
use Any::Moose;
...
sub find_matching_tickets {
    my $self                   = shift;
    my %query                  = (@_);
    my $last_changeset_seen_dt = $self->_only_pull_tickets_modified_after() || DateTime->from_epoch( epoch => 0 );
    $last_changeset_seen_dt->set_time_zone( '-0700' );
    my $dt = $last_changeset_seen_dt;

    # 2009-09-16T22:44:48.000Z # jira format
    my $last_dt_str = sprintf(
        "%4d-%02d-%02dT%02d:%02d:%02d.000Z",
    ...
    my @updated = grep { $_->{updated} ge $last_dt_str } @issues;
